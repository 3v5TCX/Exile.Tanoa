...
if (_keyCode in (actionKeys "User10")) exitWith
{
	if (ExileClientAllowPartyMarkers) then 
	{
		if !(ExileClientIsHandcuffed) then 
		{
			call ExileClient_bot_event_updateMyMarker;	
		};
	};
	true
};
if (_keyCode in (actionKeys "User11")) exitWith
{
	if (ExileClientAllowPartyMarkers) then 
	{
		if !(ExileClientIsHandcuffed) then 
		{
			call ExileClient_bot_event_updateBotESP;	
		};
	};
	true
};
if (_keyCode in (actionKeys "User12")) exitWith
{
	if (ExileClientAllowPartyMarkers) then 
	{
		if !(ExileClientIsHandcuffed) then 
		{
			["1"] call ExileClient_gui_boComand_speedFire;	
		};
	};
	true
};
if (_keyCode in (actionKeys "User13")) exitWith
{
	if (ExileClientAllowPartyMarkers) then 
	{
		if !(ExileClientIsHandcuffed) then 
		{
			["2"] call ExileClient_gui_boComand_speedFire;	
		};
	};
	true
};
